"""Concept Bottleneck Model baseline for VinDr-CXR."""
...
class ImplicitCBM(nn.Module):
    ...
    def __init__(self, num_classes: int) -> None:
        ...
        self.bottleneck = nn.Linear(1024, num_classes)
        self.head = nn.Linear(num_classes, num_classes)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass of implicit CBM.

        Args:
            x: Input image tensor.

        Returns:
            Tuple of concept logits and task logits.
        ...
        concept_logits = self.bottleneck(feats)
        concept_probs = torch.sigmoid(concept_logits)
        output_logits = self.head(concept_probs)
        return concept_logits, output_logits
...
def _train_one_epoch(
    ...
    """Train CBM for one epoch.

    Args:
        model: CBM model.
        loader: Training dataloader.
        optimizer: Optimizer.
        criterion: BCE criterion.
    ...
    for images, labels in loader:
        ...
        concept_logits, output_logits = model(images)
        loss = criterion(concept_logits, labels) + criterion(output_logits, labels)
        loss.backward()
...
def _validate(
    ...
    """Validate CBM on one dataloader.
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            loss = criterion(concept_logits, labels) + criterion(output_logits, labels)
            meter.update(float(loss.item()), n=images.shape[0])
            all_probs.append(torch.sigmoid(output_logits).cpu())
    ...
    try:
        ...
        auc = float("nan")
    return {"loss": meter.avg, "auc": auc}


def _test_auc(model: ImplicitCBM, loader: torch.utils.data.DataLoader, device: torch.device) -> float:
    """Compute macro test AUC for CBM.
...
def main() -> None:
    """Train CBM baseline across all configured seeds.
    ...
    for seed in config.SEEDS:
        set_seed(seed)
        model = ImplicitCBM(config.NUM_CLASSES).to(device)
    ...
    values = list(per_seed_auc.values())
